...
# set x range
ax1.set_xlim([-1,1])
# plt.savefig('../figures/eig_GPD_prop3d.svg', format='svg')
# plt.show()
...
eig_pi54 = np.linalg.eigvals(-np.kron(L,ang2mat(5*np.pi/4,D)))
...
ax1.scatter(eig_pi54.real, eig_pi54.imag, color=c_blue, marker = '^', s=marker_size)
ax1.scatter(eig_org.real, eig_org.imag, marker = 'x', color='black',s= cross_size)
ax1.grid(True)
ax1.axis('equal')
ax1.tick_params(axis='both', which='major', labelsize=tick_label_size)
plt.tight_layout()
plt.savefig('../figures/eig_Lap_prop3d.svg', format='svg')
plt.show()
