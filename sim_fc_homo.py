...
if __name__ == '__main__':
    ...
    # unambiguous case
    HH = np.eye(N*D)
    error = afc(ITR, dt, HH, LL, N, D, z_init)

    Errors = np.zeros((4,ITR))
    # ambiguity matrix
    HH = np.kron(np.eye(N), ang2mat(np.pi / 4, D))
    Errors[0,:]= afc(ITR, dt, HH, LL, N, D, z_init)

    # ambiguity matrix
    HH = np.kron(np.eye(N), ang2mat(np.pi / 3, D))
    Errors[1,:] = afc(ITR, dt, HH, LL, N, D, z_init)

    # ambiguity matrix
    HH = np.kron(np.eye(N), ang2mat(np.pi / 2, D))
    Errors[2,:] = afc(ITR, dt, HH, LL, N, D, z_init)

    # ambiguity matrix
    HH = np.kron(np.eye(N), ref2mat(0, D))
    Errors[3,:] = afc(ITR, dt, HH, LL, N, D, z_init)
